...
// Import modules
import KarmiaStorageAdapterMemory = require("../");
...
describe('karmia-storage-adapter-memory', function () {
    describe('getConnection', function () {
        it('Should not get connection', function (done) {
            const storages = new KarmiaStorageAdapterMemory(options);
        ...
        it('Should get existing connection', function (done) {
            const connection = {name: 'TEST_CONNECTION'},
                storages = new KarmiaStorageAdapterMemory(options, connection);
        ...
        describe('Should disconnect from database', function () {
            describe('Connected', function () {
                ...
                it('Callback', function (done) {
                    const storages = new KarmiaStorageAdapterMemory(options);
                    storages.connect().then(function () {
                        storages.disconnect(function (error?: Error, result?: any) {
                            if (error) {
                                return done(error);
                            }
            ...
            describe('Not connected', function () {
                it('Promise', function (done) {
                    const storages = new KarmiaStorageAdapterMemory(options);
                    storages.disconnect().then(function (result?: any) {
                        expect(result).to.be(undefined);
    ...
    describe('storage', function () {
        ...
        before(function (done) {
            storages.connect().then(function () {
                ...
                return Fixture.reduce(function (promise: Promise<any>, data: Item): Promise<any> {
        ...
        describe('store', function () {
            ...
            describe('Should overwrite old value', function () {
                it('Promise', function (done) {
                    ...
                    expect(storage.buffer.length).to.be(0);
                    Array.apply(null, {length: size}).map(Number.call, Number).reduce(function (collection: Promise<any>, value: number) {
        ...
        describe('Should set adapter size to unlimited', function () {
            it('Promise', function (done) {
                ...
                Array.apply(null, {length: size + 1}).map(Number.call, Number).reduce(function (collection: Promise<any>, value: number) {
                    return storage.set(value, value);
                }, Promise.resolve()).then(function () {
